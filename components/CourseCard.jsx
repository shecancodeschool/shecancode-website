    ...
    <div className='flex flex-col sm:w-[48%] md:w-[22%] shadow-md mb-10 rounded-md bg-white'>
      ...
      <div className='flex flex-col py-6 px-4 text-black justify-between min-h-72 gap-2 flex-wrap'>
        ...
        <div className='flex justify-between items-center'>
          <span>{duration}</span>
          <span className='ml-4'>{price} {price !== "Free" && "Rwf"}</span>
        </div>
        <div className='flex flex-col gap-2'>
          <Link href={link} className='w-full text-center bg-sky-600 hover:bg-sky-700 text-white text-sm rounded-2xl p-1'>Enroll Now</Link>
          <Link href={link} className='w-full text-sky-600 text-center hover:text-sky-900'>Read more</Link>
        </div>
      </div>
    </div>
  )
